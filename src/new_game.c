...
static void WarpToTruck(void)
{
    if (gSaveBlock2Ptr->playerGender == MALE)
        SetWarpDestination(MAP_GROUP(MAP_LITTLEROOT_TOWN_INTRO_HOUSE), MAP_NUM(MAP_LITTLEROOT_TOWN_INTRO_HOUSE), -1, -1, -1);
    else
        SetWarpDestination(MAP_GROUP(MAP_LITTLEROOT_TOWN_MAYS_HOUSE_2F), MAP_NUM(MAP_LITTLEROOT_TOWN_MAYS_HOUSE_2F), -1, -1, -1);
    WarpIntoMap();
}
...
void NewGameInitData(void)
{
    ...
    if (gSaveBlock2Ptr->playerGender == MALE)
    {
        ...
        FlagSet(FLAG_HIDE_PORYGON_INTRO);
    ...
    FlagClear(FLAG_COMPLETED_INTRO_SEQUENCE);
}
